...
2. **About** — personal introduction and KRIXORA brand story.
3. **Philosophy** — Problem Solving, Creative Thinking, Attention to Detail and Continuous Learning.
4. **Process** — the five-step "Idea to Shipped" workflow behind every project.
5. **Projects** — selected real-world work with live demos, repositories and case studies.
6. **Skills** — categorized technologies and tools.
7. **KRIXORA Terminal** — interactive developer console to explore the portfolio via commands.
8. **Phone Lab** — phone mockup with boot sequence and installable app grid (phone-first origin story).
9. **Journey** — development and learning timeline.
10. **Certifications** — completed courses and workshops, with a swipeable certificate slider.
11. **Toolbox** — everyday tools used to design, develop and ship projects.
12. **CTA** — a focused call-to-action bridging into contact.
13. **Contact** — social links and collaboration/contact interface.
14. **Footer** — navigation, brand statement and supporting links.
...
**Repository / GitHub:** https://github.com/KRIXORA/Flowora
...
**Repository / GitHub:** https://github.com/KRIXORA/Cashora
...
**Repository / GitHub:** https://github.com/KRIXORA/Coursora
...
- **KRIXORA Terminal** — command-line style explorer (`whoami`, `ls projects`, `open flowora`, `neofetch`, and more)
- **Phone Lab** — cinematic phone boot + home screen with real product app icons
- Project cards with 3D tilt, shine sweep and case-study modal (Problem → Approach → Result → Learnings)
...
- Project image interactions and tilt
...
│   ├── icons/
│   │   ├── krixora-mark.svg
│   │   └── favicon-16 / -48 / -180 / -192 / -512 / favicon.png
│   │
│   └── resume/
│       └── Krish-Parmar-Resume.pdf
│
├── README.md
├── LICENSE
├── robots.txt
...
### Getting indexed & ranking (beyond the code)

The on-page SEO above makes the site *readable* to search engines — it doesn't by itself make it rank. Once deployed:

- Submit the site + `sitemap.xml` to [Google Search Console](https://search.google.com/search-console) and [Bing Webmaster Tools](https://www.bing.com/webmasters). Un-submitted pages can take weeks to be crawled naturally.
- Verify the `google-site-verification` meta tag in `index.html` matches the property in Search Console (or re-verify if the domain changes).
- Get real backlinks pointing to the site — GitHub profile README, LinkedIn "Featured" section, Flowora/Cashora/Coursora repo READMEs, Dev.to / Hashnode posts, etc. Backlinks matter more for ranking than any meta tag.
- A custom domain (e.g. `krish.dev`, `krixora.com`) ranks and brands better long-term than a `.vercel.app` subdomain — worth prioritizing once available.
- Keep publishing genuine, unique content (case studies, blog posts about Flowora/Cashora/Coursora) — search engines rank sites with real, updated content over static one-pagers, however well-optimized.
- Core Web Vitals / Lighthouse score matters for ranking too, not just accessibility — worth a periodic audit as the site grows.
...
- deeper PWA improvements (offline caching / service worker)
- Instagram profile integration (pending account creation)
- additional performance audits
- Lighthouse/accessibility optimization
- move to a custom domain (stronger branded search presence than a `.vercel.app` subdomain)

Already shipped in the current build: interactive Terminal, Phone Lab, project tilt + case studies, dark/light theme, certificate slider.
